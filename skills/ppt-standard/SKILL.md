...
This skill is **self-contained** — no dependency on `u1-image-base` for LLM/VLM (T2I still goes through `u1-image-base`). Every call through `$SKILL_DIR/scripts/run_stage.py`. Every subcommand is deterministic: one input set → one output artifact → one-line JSON status.
...
- Does not call `u1-image-base` for LLM/VLM (only for T2I).
