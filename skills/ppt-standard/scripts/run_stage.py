...
def cmd_gen_image(deck: Path, page_no: int, slot_id: str) -> int:
    """Generate a single slot's image via u1-image-base's openclaw_runner (T2I).

    Policy: T2I must route through u1-image-base, NOT through model_client.
    ...
    # Locate u1-image-base/openclaw_runner.py
    u1_base = os.environ.get("U1_IMAGE_BASE", "").strip()
    if u1_base:
        runner = Path(u1_base) / "u1_image_base" / "openclaw_runner.py"
    else:
        # fallback: assume sibling dir under skills/
        runner = SKILL_DIR.parent / "u1-image-base" / "u1_image_base" / "openclaw_runner.py"
    if not runner.exists():
        return _fail(f"u1-image-base openclaw_runner.py not found at {runner}; set $U1_IMAGE_BASE")

    save_path = deck / slot["local_path"]
    save_path.parent.mkdir(parents=True, exist_ok=True)

    cmd = [
        sys.executable, str(runner), "u1-image-generate",
    ...
    try:
        ...
        return _record_failure("openclaw_runner u1-image-generate timed out after 600s")
    except FileNotFoundError as e:
        return _record_failure(f"failed to spawn python for u1-image-base: {e}")
    ...
    if not save_path.exists() or save_path.stat().st_size == 0:
        return _record_failure(f"u1-image-generate returned ok but {save_path} is missing/empty")
