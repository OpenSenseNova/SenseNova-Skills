...
from u1_image_base.generation.core import ensure_output_path
from u1_image_base.generation.core.client_base import (
...
class SensenovaText2ImageClient(T2IBaseClient):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        """
        api_key = api_key or global_configs.U1_API_KEY
        if not api_key:
            raise MissingApiKeyError(
                "API key is missing: {}".format(global_configs.get_env_var_help("U1_API_KEY"))
            )
        base_url = base_url or global_configs.U1_IMAGE_GEN_BASE_URL
        if not base_url:
...
if __name__ == "__main__":
    ...
    async def main_async():
        client = SensenovaText2ImageClient(
            api_key=global_configs.U1_API_KEY,
            base_url=global_configs.U1_IMAGE_GEN_BASE_URL,
        )
