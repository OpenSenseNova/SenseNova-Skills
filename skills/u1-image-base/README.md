...
- `u1-image-generate`: image generation
- `u1-image-recognize`: image recognition (VLM)
- `u1-text-optimize`: text optimization (LLM)
...
```ini
# Image generation
U1_API_KEY="<sensenova-token-plan-api-key>"
U1_IMAGE_GEN_MODEL="sensenova-u1-fast"   # or other image generation models available in the SenseNova Token Plan
# LLM/VLM
U1_LM_API_KEY="<sensenova-token-plan-api-key>"
U1_LM_MODEL="sensenova-6.7-flash-lite"   # or other LLM/VLM models available in the SenseNova Token Plan
```
...
> For experienced developers, see [configs.py](u1_image_base/configs.py) for the full list of variables and defaults.
>
...
| ---------- | ----------- | ------- |
| `U1_API_KEY` | The API key for the SenseNova Token Plan | (Required) |
| `U1_IMAGE_GEN_MODEL_TYPE` | The type of image generation model to use | `"sensenova"` |
...
1. Set `U1_IMAGE_GEN_MODEL_TYPE` according to the model type, available values are:
    ...
    U1_IMAGE_GEN_MODEL_TYPE="openai-image"
    ```
...
2. Set `U1_IMAGE_GEN_BASE_URL` to the base URL for the image generation API. For example:
    ...
    # For OpenAI's image generation API
    U1_IMAGE_GEN_BASE_URL="https://api.openai.com/v1"
    ```
...
3. Set `U1_IMAGE_GEN_MODEL` to the model name provided by the model type. For example:
    ...
    # For OpenAI's image generation API
    U1_IMAGE_GEN_MODEL="gpt-image-2"
    ```

4. (**Required**) Set `U1_API_KEY` to the API key for the image generation API.

    ```ini
    U1_API_KEY="sk-your-image-generation-api-key"
    ```

#### VLM and LLM

If you're not intended to use different models for VLM and LLM, you can use the same `U1_LM_*` variables to configure both VLM and LLM, and skip the following VLM and LLM configurations.

##### Use the same VLM and LLM models

Full configuration for VLM and LLM (with shared `U1_LM_*` environment variables):

| Config Keys | Description | Default |
| ----------- | ----------- | ------- |
| `VLM_API_KEY` & `LLM_API_KEY` (via `U1_LM_API_KEY` env var) | The API key for the VLM and LLM API | (Required) |
| `VLM_BASE_URL` & `LLM_BASE_URL` (via `U1_LM_BASE_URL` env var) | The base URL for the VLM and LLM API | `"https://token.sensenova.cn/v1"` |
...
1. Set `VLM_TYPE` & `LLM_TYPE` (via `U1_LM_TYPE`) according to the shared LM API interface type. Available values are:

    ```ini
    # (Default) OpenAI-compatible `/chat/completions` interface (most widely supported)
    U1_LM_TYPE="openai-completions"
    # Anthropic Messages `/messages` interface
    U1_LM_TYPE="anthropic-messages"
    ```
...
2. Set `VLM_BASE_URL` & `LLM_BASE_URL` (via `U1_LM_BASE_URL`) to the shared LM endpoint base URL. For example:
    ...
    U1_LM_BASE_URL="https://api.openai.com/v1"
    # For Google Gemini API (OpenAI-compatible)
    U1_LM_BASE_URL="https://generativelanguage.googleapis.com/v1beta/openai/"
    ```

3. Set `VLM_MODEL` & `LLM_MODEL` (via `U1_LM_MODEL`) to the shared LM model name. For example:

    ```ini
    # (Default) SenseNova 6.7 Flash Lite
    U1_LM_MODEL="sensenova-6.7-flash-lite"
    # Anthropic Claude Sonnet 4.6
    U1_LM_MODEL="claude-sonnet-4-6"
    # Google Gemini 3 Flash Preview
    U1_LM_MODEL="gemini-3-flash-preview"
    # OpenAI GPT 5.5
    U1_LM_MODEL="gpt-5.5"
    ```

4. (**Required**) Set `VLM_API_KEY` & `LLM_API_KEY` (via `U1_LM_API_KEY`) to the API key for the shared LM endpoint.

    ```ini
    U1_LM_API_KEY="sk-your-api-key"
    ```
...
>
> - For VLM model: if both `VLM_MODEL` and `U1_LM_MODEL` are set, `VLM_MODEL` wins.
> - For LLM model: if both `LLM_MODEL` and `U1_LM_MODEL` are set, `LLM_MODEL` wins.
>
...
Use the above environment variables to override the `U1_LM_*` variables for VLM and LLM.
...
- Fix: set `U1_API_KEY` for image generation, and set either `U1_LM_API_KEY` or task-specific keys (`VLM_API_KEY` / `LLM_API_KEY`).

### Wrong base URL

- Symptom: request fails immediately, or URL validation/auth endpoint errors.
- Fix: verify `U1_IMAGE_GEN_BASE_URL`, `U1_LM_BASE_URL`, `VLM_BASE_URL`, `LLM_BASE_URL` are full base URLs (with scheme + host), for example `https://token.sensenova.cn/v1`.
