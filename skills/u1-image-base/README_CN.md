...
- `u1-image-generate`：图像生成
- `u1-image-recognize`：图像识别（VLM）
- `u1-text-optimize`：文本优化（LLM）
...
U1_IMAGE_GEN_MODEL="sensenova-u1-fast"   # 或 Token Plan 中可用的其他图像生成模型
# LLM/VLM
U1_LM_API_KEY="<sensenova-token-plan-api-key>"
U1_LM_MODEL="sensenova-6.7-flash-lite"   # 或 Token Plan 中可用的其他 LLM/VLM 模型
```
...
> 进阶开发者可查看 [configs.py](u1_image_base/configs.py) 获取完整变量与默认值。
>
> 便于快速追踪行为的关键符号：
>
...
| 配置键 | 说明 | 默认值 |
| ------ | ---- | ------ |
| `U1_API_KEY` | SenseNova Token Plan 的 API Key | （必填） |
| `U1_IMAGE_GEN_MODEL_TYPE` | 图像生成模型类型 | `"sensenova"` |
...
4. （必填）设置 `U1_API_KEY` 为图像生成 API 的密钥：
...
若你不打算为 VLM 和 LLM 分别使用不同模型，可直接使用同一组 `U1_LM_*` 环境变量统一配置，并跳过后续拆分配置部分。

##### 使用相同的 VLM 和 LLM 模型

使用共享 `U1_LM_*` 环境变量时，VLM/LLM 的完整配置如下：

| 配置键 | 说明 | 默认值 |
| ------ | ---- | ------ |
| `VLM_API_KEY` 与 `LLM_API_KEY`（通过 `U1_LM_API_KEY` env var） | VLM 与 LLM API Key | （必填） |
| `VLM_BASE_URL` 与 `LLM_BASE_URL`（通过 `U1_LM_BASE_URL` env var） | VLM 与 LLM API 的基础 URL | `"https://token.sensenova.cn/v1"` |
...
1. 按 VLM/LLM 的 API 类型设置 `VLM_TYPE` 与 `LLM_TYPE`（配置 `U1_LM_TYPE` 环境变量）。可选值如下：
...
3. 将 `VLM_MODEL` 与 `LLM_MODEL`（配置 `U1_LM_MODEL` 环境变量）设置为 VLM/LLM 模型名，例如：
...
4. （必填）设置 `VLM_API_KEY` 与 `LLM_API_KEY`（配置 `U1_LM_API_KEY` 环境变量）为 VLM/LLM API 的密钥：
...
>
> - 对于 VLM 模型：若同时设置 `VLM_MODEL` 与 `U1_LM_MODEL`，则 `VLM_MODEL` 生效。
> - 对于 LLM 模型：若同时设置 `LLM_MODEL` 与 `U1_LM_MODEL`，则 `LLM_MODEL` 生效。
>
> 例如：
>
> ```ini
...
以上独立变量可用于覆盖 `U1_LM_*` 对 VLM/LLM 的共享配置。
...
- 处理：图像生成需设置 `U1_API_KEY`；VLM/LLM 需设置 `U1_LM_API_KEY`，或分别设置 `VLM_API_KEY` / `LLM_API_KEY`。

### base URL 配置错误

- 现象：请求立即失败，或出现 URL 校验 / endpoint 相关错误。
- 处理：检查 `U1_IMAGE_GEN_BASE_URL`、`U1_LM_BASE_URL`、`VLM_BASE_URL`、`LLM_BASE_URL` 是否为完整基础 URL（包含 scheme + host），例如 `https://token.sensenova.cn/v1`。
