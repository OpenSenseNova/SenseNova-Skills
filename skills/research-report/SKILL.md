...
description: 研究报告写作与修改；当需要把 deep research 的判断层写成最终 `report.md`，或单独对已有报告/文稿做重写、改写、润色、重组结构、增强摘要、补充表格、规划插图并调用 `u1-image-base` 生成合适配图时使用。适用于完整终稿生成，也适用于已有草稿的定向编辑，不负责从零重新做研究综合判断。
---
...
- 用表格、时间线、Mermaid 或图片提升可读性；需要 AI 配图时，调用 `u1-image-base` 生成图片并嵌入报告。
- 清楚呈现条件、限制、冲突和不确定性。
...
需要 AI 配图时，使用底层 skill `u1-image-base`，不是 `u1-base-image`。

解析 `<U1_IMAGE_BASE>`：

1. 优先使用环境变量 `U1_IMAGE_BASE`。
2. 否则在当前 skills 根目录或相邻目录查找 `u1-image-base/`。
3. 找不到时，不要假装已生成图片；在报告中先使用 Mermaid/表格替代，并在最终说明缺少 `u1-image-base` 路径。
...
9. **生成与嵌入配图**：对需要 AI 配图的位置，调用 `u1-image-base` 生成图片，保存到 `{report_dir}/images/`，并在对应章节用 Markdown 相对路径嵌入。
...
- 逐句润色很多，但结构问题完全没解决。
- 用图片做装饰，不能帮助理解。
- 只写“可加入配图”，但没有调用 `u1-image-base` 生成文件。
- 生成了图片文件，却没有在 `report.md` 的合适章节嵌入。
- 把 `u1-image-base` 误写成 `u1-base-image`，导致找不到依赖。
