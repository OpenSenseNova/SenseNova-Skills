...
- **可用工具**：`web_search`、`web_fetch`、`read`、`write`、`exec`。
  - 搜索统一用 `web_search`，OpenClaw 按配置的 provider 路由。**下文中 `serper_search` / `brave_search` / `tavily_search` 都已重命名为 `web_search`**。时间过滤参数名因 provider 而异：brave→`freshness`（pd/pw/pm/py）、tavily→`time_range`（day/week/month/year）、exa→`start_published_date`；按 tool 响应提示使用。
  - `image_search` 在 OpenClaw 中不是独立工具——需要图片时用 `web_search` 搭配图片站点关键词，或用 provider 特有参数。若不支持则跳过图片搜索。
- **没有 `ask_user`**：任何不清楚的地方都应在子报告中如实标注"证据不足"，不要尝试问用户。
- **文件传递用绝对路径**：按 controller 的 `**输出路径**` 用 `write` 写入子报告；修订时覆写同一路径。
- 可用 skill：`search-code` / `search-academic` / `search-social-cn` / `search-social-en`。Python 脚本通过 `exec` 运行
...
**通用搜索**（web_search / web_search / web_search）：
...
| **search-social-cn** | 知乎、B站、抖音平台内搜索（Google 仅索引冰山一角） | sources 含 `social_media` / `review`（中文）：获取真实用户评价和讨论 |
...
2. **再对时效敏感的子问题追加限时搜索**：针对需要最新信息的子问题，额外做一轮带时间过滤的搜索
   - web_search: 用 `tbs` 参数（`h`=小时, `d`=天, `w`=周, `m`=月, `y`=年）
   - web_search: 用 `freshness` 参数（`pd`=天, `pw`=周, `pm`=月, `py`=年）
   - web_search: 用 `time_range` 参数（`day`/`week`/`month`/`year`），可结合 `topic="news"` 搜新闻
...
- **追溯原始来源**：搜到的内容引用了原始报告、数据源或关键人物 → 用 web_fetch 获取一手来源
...
1. 使用 `write` 将完整的 Markdown 子报告写入指定的**绝对路径**（直接使用 controller 提供的路径，不要修改或拼接）
2. 回复 controller 确认写入完成，附上文件路径（不要在回复中包含子报告全文）

修订子报告时，同样将修订版使用 `write` 覆写到指定路径。

controller 消息中提供的 `read` 路径（如前置维度子报告）也是绝对路径，直接使用即可。
