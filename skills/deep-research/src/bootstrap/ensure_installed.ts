...
function emptySummary(): BootstrapSummary {
  return {
...
async function copySkill(plan: Awaited<ReturnType<typeof buildAssetPlan>>["skills"]["toCopy"][number]) {
...
async function copyWorkspace(
  plan: Awaited<ReturnType<typeof buildAssetPlan>>["workspaces"]["toCopy"][number],
) {
...
export async function ensureInstalled(
  ...
  try {
    ...
    const versionChanged =
      previousState?.success === true && previousState.pluginVersion !== params.pluginVersion;
    ...
    for (const workspace of assetPlan.workspaces.toCopy) {
      (workspace.overwrite ? summary.workspaces.overwritten : summary.workspaces.copied).push(
        workspace.agentId,
      );
    }
