...
function ensureObject(root: JsonObject, ...keys: string[]): JsonObject {
...
function mergeAllowAgents(existing: string[] | undefined, incoming: readonly string[]): string[] {
...
function upsertAgentsList(existing: JsonObject[] | undefined, force: boolean) {
...
function mergePluginEntry(existing: JsonObject | undefined) {
...
export function mergeDeepResearchConfig(input: JsonObject, options: MergeOptions): MergeResult {
  ...
  for (const key of ["maxSpawnDepth", "maxConcurrent", "runTimeoutSeconds"] as const) {
    if (!(key in subagents)) {
