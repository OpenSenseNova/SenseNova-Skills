...
export default definePluginEntry({
    ...
    register(api) {
        ...
        api.registerTool(deepResearchInstallTool({
            pluginRoot,
            openclawHome,
            pluginVersion,
        }));
        ...
        void ensureInstalled({
            ...
            api.logger?.warn?.(`deep-research bootstrap failed: ${error instanceof Error ? error.message : String(error)}`);
        });
        api.logger?.info?.("deep-research plugin registered (prepare_report_citations + volcengine-image provider, bootstrap)");
    },
});
