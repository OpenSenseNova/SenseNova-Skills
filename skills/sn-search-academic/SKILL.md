...
description: 搜索学术论文和百科知识，列出论文章节，读取论文全文或章节，追踪 references/citations 引用树。适合学术调研、论文精读、相关工作梳理和引用链追溯。
---
...
- 不要并行启动多个本技能脚本；`search.py` 和 `refTree.py` 内部已经处理并发、超时和 provider fallback。
...
统一引用树入口。`--paper_id` 和 `--title` 都必填；标题用于 fallback 精确匹配。
