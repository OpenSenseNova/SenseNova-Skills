...
完成 briefing 所需字段后，读取 `{plugin_skills_dir}/sn-report-format-discovery/SKILL.md` 并严格执行。向它提供原始 `query`、`task_interpretation`、目标读者、使用场景、用户已表达的格式要求，以及可选的 `format_revision_request`。
