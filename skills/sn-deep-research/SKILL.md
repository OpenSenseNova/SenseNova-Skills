...
**语言锚定（全档位硬约束）**：controller 根据原始 query 只解析一次输出语言，并将 `language` 作为请求级参数传给所有角色；角色不得重新推断语言。用户明确指定的输出语言优先，否则使用 query 的主要指令语言。
...
3. **语言锚点必传**：每条含 `language:{language}`；role 必须使用该参数。
