...
All API calls in this skill are executed through the `openclaw_runner.py` of the `u1-image-base` skill, with authentication parameters using default values (CLI > environment variables > built-in defaults),无需显式传入。
...
2. Send uniform preflight message: `"Using u1-infographic skill to generate infographic, please wait..."`
...
Worker Agent receives `user_prompt`, `max_rounds`, `output_mode`, `prompts_expand_mode`, and the working directory of this skill (`U1_IMAGE_INFOG`).

#### Step 0 — Initialization

1. Generate `task_id` (using timestamp, format `YYYYMMDD_HHMMSS`)
2. Create a uniform temporary directory: `/tmp/openclaw/u1-infographic/<task_id>/` as `TEMP_DIR`
...
```json
{
  ...
  "timing": {
    "image_generation": { "elapsed_seconds": 12.34, "model": "u1_image_model" },
    "vlm_review": { "elapsed_seconds": 5.67, "model": "sensenova-122b-128k-step9k" }
  }
}
```
...
```json
{
  ...
  "timing": {
    ...
    "prompt_detection": { "elapsed_seconds": 2.11, "model": "sensenova-122b-128k-step9k" },
    "content_analysis": { "elapsed_seconds": 3.22, "model": "sensenova-122b-128k-step9k" },
    "prompt_expand": { "elapsed_seconds": 8.45, "model": "sensenova-122b-128k-step9k" }
  },
  "rounds": [
    {
      "round": 1,
